#!/usr/bin/env node
// codearts-cli bin entry — thin wrapper that execs the Go binary.
//
// npm links this file to the global bin directory (e.g. /usr/local/bin/codearts-cli).
// It locates the real binary under ./bin/ (placed there by scripts/install.js)
...
const bin = path.join(__dirname, "..", "bin", "codearts-cli" + ext);

// On Windows, a crashed self-update may have left the binary renamed.
...
if (!fs.existsSync(bin)) {
  console.error(
    `Error: codearts-cli binary not found at ${bin}\n\n` +
