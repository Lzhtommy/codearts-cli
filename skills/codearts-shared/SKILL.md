...
version: 0.1.1
description: "CodeArts CLI 共享基础：AK/SK 配置初始化、凭证管理、通用标志（--project-id / --dry-run）、端点解析、错误处理。首次使用 codearts-cli 或遇到认证/配置问题时触发。"
metadata:
  category: "devops"
  requires:
    bins: ["codearts-cli"]
...
| 标志           | 说明                                          |
| -------------- | --------------------------------------------- |
| `--project-id` | 流水线命令**必填**；工作项命令可选（从 config 兜底） |
